...
List wrap_collapse_networks(List network_r) {
  ...
  // sort networks (IPv4 & IPv6 handled independently)
  std::vector<asio::ip::network_v4> sorted_networks_v4;
  std::vector<asio::ip::network_v6> sorted_networks_v6;
  for (std::size_t i=0; i<vsize; ++i) {
    ...
    if (network.is_na[i]) {
      ...
      sorted_networks_v6.push_back(network.network_v6[i]);
    } else {
      sorted_networks_v4.push_back(network.network_v4[i]);
    }
  }
  std::sort(sorted_networks_v4.begin(), sorted_networks_v4.end(),
            [](asio::ip::network_v4 a, asio::ip::network_v4 b) { return a.address() < b.address(); });
  std::sort(sorted_networks_v6.begin(), sorted_networks_v6.end(),
            [](asio::ip::network_v6 a, asio::ip::network_v6 b) { return a.address() < b.address(); });
  ...
  // identify contiguous address ranges (IPv4)
  if (sorted_networks_v4.size() > 0) {
    auto cur_start = sorted_networks_v4[0].address();
    auto cur_end = broadcast_address<asio::ip::address_v4>(sorted_networks_v4[0]);

    for (std::size_t i=1; i<sorted_networks_v4.size(); ++i) {
      if (i % 10000 == 0) checkUserInterrupt();

      if (sorted_networks_v4[i].address() <= advance_ip(cur_end, 1)) {
        auto next_end = broadcast_address<asio::ip::address_v4>(sorted_networks_v4[i]);
        if (next_end >= cur_end) {
          cur_end = next_end;
        }
      } else {
        auto networks = summarize_address_range<asio::ip::network_v4>(cur_start, cur_end);
        std::copy(networks.begin(), networks.end(), std::back_inserter(out_network_v4));
        std::fill_n(std::back_inserter(out_network_v6), networks.size(), asio::ip::network_v6());
        std::fill_n(std::back_inserter(out_is_ipv6), networks.size(), false);
        std::fill_n(std::back_inserter(out_is_na), networks.size(), false);

        cur_start = sorted_networks_v4[i].address();
        cur_end = broadcast_address<asio::ip::address_v4>(sorted_networks_v4[i]);
      }
    }
  ...
  // identify contiguous address ranges (IPv6)
  if (sorted_networks_v6.size() > 0) {
    auto cur_start = sorted_networks_v6[0].address();
    auto cur_end = broadcast_address<asio::ip::address_v6>(sorted_networks_v6[0]);

    for (std::size_t i=1; i<sorted_networks_v6.size(); ++i) {
      if (i % 10000 == 0) checkUserInterrupt();

      if (sorted_networks_v6[i].address() <= advance_ip(cur_end, 1)) {
        auto next_end = broadcast_address<asio::ip::address_v6>(sorted_networks_v6[i]);
        if (next_end >= cur_end) {
          cur_end = next_end;
        }
      } else {
        auto networks = summarize_address_range<asio::ip::network_v6>(cur_start, cur_end);
        std::copy(networks.begin(), networks.end(), std::back_inserter(out_network_v6));
        std::fill_n(std::back_inserter(out_network_v4), networks.size(), asio::ip::network_v4());
        std::fill_n(std::back_inserter(out_is_ipv6), networks.size(), true);
        std::fill_n(std::back_inserter(out_is_na), networks.size(), false);

        cur_start = sorted_networks_v6[i].address();
        cur_end = broadcast_address<asio::ip::address_v6>(sorted_networks_v6[i]);
      }
    }
