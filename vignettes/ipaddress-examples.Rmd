...
A very common task is to check if an IP address is within an IP network, so the ipaddress package provides a couple of different functions to help with this workflow: `is_within()` and `is_within_any()`.
...
We can do that by performing a cross-join with the table of networks, and filtering those rows that match.
