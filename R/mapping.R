...
#' embed_ipv4_mapped(extract_ipv4_mapped(x, keep_ipv6 = TRUE))
...
#' `extract_ipv4_mapped()`
#'
#' `extract_ipv4_mapped()` extracts an IPv4 address from an IPv4-mapped IPv6 address
#' @rdname mapping
#' @export
...
#' `embed_ipv4_mapped()`
#'
#' `embed_ipv4_mapped()` embeds an IPv4 address within an IPv4-mapped IPv6 address
#' @rdname mapping
#' @export
embed_ipv4_mapped <- function(x) {
  assertthat::assert_that(is_ip_address(x))

  new_ip_address_encode(embed_ipv4_mapped_wrapper(x))
}
